...
function SearchBar() {
  ...
  return (
    <div className='relative  max-w-md mx-auto searchbox-wrapper'>
        <input type='text ' value={searchText} onFocus={()=>setIsActive(true)} onChange={(event)=>textSearch(event)} placeholder='  Search Manga ' className=' text-white rounded-sm p-2 font-stretch-normal outline-none inset-shadow-sm inset-shadow-gray-950  bg-zinc-800 h-[35px] w-[390px]  '/>
        {isActive&& suggestions.length>0 && (
            <ul className='absolute top-full mt-1 opacity-98 bg-purple-600 rounded-lg w-[390px] transition-all duration-300 ease-in-out hover:scale-105 hover:shadow-xl shadow-purple-900 '>
                {suggestions.map((manga)=>
                (
                    <li className='text-white  p-2 font-stretch-normal opacity-100 rounded-sm hover:bg-purple-950  mt-1  transition '>
